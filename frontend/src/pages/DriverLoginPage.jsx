...
const DRIVER_PHONE_MAX_DIGITS = 12;
const DRIVER_PHONE_STANDARD_ERROR =
  "Use a valid Ethiopian phone number: 09XXXXXXXX or 2519XXXXXXXX.";

function isDriverPhoneStandard(value) {
...
export function DriverLoginPage() {
  ...
  const validatePhone = (value) => {
    const digitsOnly = String(value ?? "")
      .replace(/\D/g, "")
      .slice(0, DRIVER_PHONE_MAX_DIGITS);
    const uiValue = formatEthiopianPhoneForInput(digitsOnly);
    setPhone(uiValue);
    if (!uiValue) {
      setPhoneError("");
      return;
    ...
    setPhoneError(isDriverPhoneStandard(normalized) ? "" : DRIVER_PHONE_STANDARD_ERROR);
  };
  ...
  async function handleSubmit(e) {
    ...
    if (!isDriverPhoneStandard(phoneValue)) {
      setPhoneError(DRIVER_PHONE_STANDARD_ERROR);
      setError(DRIVER_PHONE_STANDARD_ERROR);
      return;
    }
    setPhoneError("");
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <Input
              ...
              inputMode="numeric"
              pattern="[0-9]*"
              maxLength={DRIVER_PHONE_MAX_DIGITS}
              autoComplete="tel"
              value={phone}
              ...
              onBlur={() => {
                ...
                setPhoneError(
                  isDriverPhoneStandard(normalized)
                    ? ""
                    : DRIVER_PHONE_STANDARD_ERROR
                );
              }}
