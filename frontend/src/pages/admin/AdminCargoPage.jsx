...
function isTripNotExpired(trip) {
...
export function AdminCargoPage() {
  ...
  const refreshCore = useCallback(async () => {
    ...
    try {
      ...
      setCargo(Array.isArray(c) ? c : Array.isArray(c?.data) ? c.data : []);
      setUsers(Array.isArray(u) ? u : Array.isArray(u?.data) ? u.data : []);
      setTrips(Array.isArray(tr) ? tr : Array.isArray(tr?.data) ? tr.data : []);
      setRoutes(
      ...
      setRoles(Array.isArray(rl) ? rl : Array.isArray(rl?.data) ? rl.data : []);
    } catch (e) {
  ...
  const tripsForAssign = useMemo(() => {
    let list = assignableTrips.filter((t) => isTripNotExpired(t));
  ...
  const tripsForAssignDropdown = useMemo(() => {
    ...
    return tripsForAssign.filter((t) => {
      ...
      const anyAlreadyOnThisTrip = selectedRows.some(
        (row) =>
          Number(row.trip_id) === tid &&
          isTripNotExpired(
            assignableTrips.find(
              (trip) => Number(trip.id) === Number(row.trip_id),
            ),
          ),
      );
      return !anyAlreadyOnThisTrip;
    });
  }, [tripsForAssign, selectedCargoIds, cargo, assignableTrips]);
  const activeTrips = useMemo(() => {
    return trips.filter(isTripNotExpired);
  }, [trips]);
  ...
  const tripSelectOptionText = useCallback(
    (t) => {
      ...
      const driverName = u
        ? String(u.full_name ?? "").trim() || "Driver"
        : t.driver_id
          ? `Driver ${t.driver_id}`
          : "No driver";
      return `${routePart} · ${dep} · ${driverName}`;
    },
    [users, routeLine],
  );
  ...
  const filteredSorted = useMemo(() => {
    ...
    rows.sort((a, b) => {
      ...
      } else if (["id", "owner_id", "trip_id", "weight", "fee"].includes(key)) {
        va = Number(va);
        vb = Number(vb);
    ...
    return rows;
  }, [cargo, search, filterStatus, sortKey, sortDir, userLabel, tripLabel]);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (err.status === 500) {
        errorMessage =
          "Server error occurred. The cargo record may have related data.";
      } else if (err.status === 403) {
        errorMessage = "You don't have permission to delete this cargo record.";
      } else if (err.status === 404) {
        errorMessage =
          "Cargo record not found. It may have already been deleted.";
  ...
  return (
    <div className="space-y-8">
      ...
      <Card>
          ...
          >
            <option value="">
              {tripsForAssign.length === 0
                ? "No active trips for this driver — pick another driver or assign a driver on Admin Trips"
                : tripsForAssignDropdown.length === 0
                  ? selectedCargoIds.length
                    ? "Checked cargo is already assigned to all active trips shown here (wait for expiry or choose another driver)"
                    : "No active trips available — check driver filter or trips list"
                  : "Select trip…"}
            </option>
        ...
        tripsForAssignDropdown.length < tripsForAssign.length ? (
          <p className="mt-1 text-xs text-slate-500">
            Hidden from the list: trips that already include at least one
            checked cargo row and have not expired yet.
          </p>
        ) : null}
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <Button
            ...
            variant="secondary"
            disabled={
              assignSubmitting || !assignTripReady || pendingCargoCount === 0
            }
      ...
      <ConfirmModal
        ...
        onConfirm={() => runBulkAssign("all", { confirm: "ASSIGN_ALL" })}
