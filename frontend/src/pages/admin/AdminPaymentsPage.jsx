...
export function AdminPaymentsPage() {
  ...
  const getUserName = (userId) => {
    ...
    return user?.full_name || (userId != null ? `User #${userId}` : "—");
  };

  const getPayerNameFromPayment = (payment) => {
    const ticket = tickets.find((t) => Number(t.id) === Number(payment?.ticket_id));
    if (!ticket) return "—";
    return (
      String(ticket?.user_full_name ?? "").trim() ||
      String(ticket?.passenger_name ?? "").trim() ||
      (ticket?.user_id != null ? `User #${ticket.user_id}` : "—")
    );
  };
  ...
  const filteredSorted = useMemo(() => {
    ...
    rows.sort((a, b) => {
      let va = a?.[sortKey];
      let vb = b?.[sortKey];
      if (sortKey === "payer_name") {
        va = getPayerNameFromPayment(a).toLowerCase();
        vb = getPayerNameFromPayment(b).toLowerCase();
      } else if (["id", "ticket_id", "amount"].includes(sortKey)) {
        va = Number(va) || 0;
        vb = Number(vb) || 0;
