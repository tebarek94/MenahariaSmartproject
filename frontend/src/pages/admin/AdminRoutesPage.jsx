  ...
  export function AdminRoutesPage() {
    ...
    async function handleCreate(e) {
      ...
      if (!lettersOnly.test(cOrigin.trim()) || !lettersOnly.test(cDest.trim())) {
        setError("Origin and destination must contain letters only.");
        return;
      }
      setSubmitting(true);
      try {
        const exists = rows.some(
          (r) =>
            r.origin?.toLowerCase() === cOrigin.trim().toLowerCase() &&
            r.destination?.toLowerCase() === cDest.trim().toLowerCase(),
        );
    ...
    async function handleUpdate(e) {
      ...
      if (
        !lettersOnly.test(origin.trim()) ||
        !lettersOnly.test(destination.trim())
      ) {
        setError("Origin and destination must contain letters only.");
        return;
      }
      ...
      try {
        const exists = rows.some(
          (r) =>
            r.id !== editingId &&
            r.origin?.toLowerCase() === origin.trim().toLowerCase() &&
            r.destination?.toLowerCase() === destination.trim().toLowerCase(),
        );

        if (exists) {
          setError(
            "Another route with same origin and destination already exists.",
          );
          return;
        }
        const body = {
    ...
    const confirmDelete = async () => {
      ...
      try {
        ...
        if (err.status === 500) {
          errorMessage =
            "Server error occurred. The route may be referenced by existing trips or have database constraints. Please delete all associated trips first or contact your database administrator to check foreign key constraints.";
    ...
    required
    pattern="[A-Za-z\s]+"
    title="Origin must contain letters and spaces only"
  />
  ...
  <Input
    ...
    required
    pattern="[A-Za-z\s]+"
    title="Destination must contain letters and spaces only"
  />
