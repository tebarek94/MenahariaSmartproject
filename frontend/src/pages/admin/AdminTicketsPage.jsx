...
export function AdminTicketsPage() {
  ...
  async function handleCreate(e) {
    ...
    const selectedTrip = trips.find((t) => String(t.id) === String(cTripId));

    if (!selectedTrip || !isTripNotExpired(selectedTrip)) {
      setError("You cannot create a ticket for an expired trip.");
      return;
    }
    setSubmitting(true);
    try {
  ...
  async function handleUpdate(e) {
    ...
    setNotice("");
    setError("");
    const { user_id, trip_id, seat_id, ticket_code, status, payment_status } =
      editForm;
    if (!user_id || !trip_id || !seat_id || !status || !payment_status) {
      setError("All fields except optional code are required for update.");
      return;
    }
    const selectedTrip = trips.find((t) => String(t.id) === String(trip_id));

    if (!selectedTrip || !isTripNotExpired(selectedTrip)) {
      setError("Cannot assign ticket to an expired trip.");
      return;
    }
    setSavingEdit(true);
    try {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (err.status === 500) {
        errorMessage =
          "Server error occurred. The ticket may have associated payments.";
  ...
  const handleDownloadTicket = async (ticketId) => {
    ...
    try {
      const tokenResponse =
        await ticketsService.generateDownloadToken(ticketId);
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-p-muted max-w-xl text-sm sm:text-[0.95rem]">
            ...
            <strong className="font-semibold text-p-heading">Add ticket</strong>{" "}
            to open the form. Passenger accounts only; pick a trip, then a seat
            on that vehicle.
          </p>
        </div>
        <Button
          ...
          >
            ...
            >
              ...
              >
                ...
                {trips.filter(isTripNotExpired).map((t) => (
              ...
              <div className="flex flex-col justify-end gap-2 sm:col-span-2 lg:col-span-1">
                ...
                {!loading && passengers.length === 0 ? (
                  <p className="text-xs text-amber-800 dark:text-amber-400/90">
                    No passenger users found. Add users with a passenger role in
                    Users.
                  </p>
                ) : null}
              </div>
