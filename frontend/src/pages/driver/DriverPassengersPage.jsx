...
import { driverAssignmentsService } from "@/services/driverAssignments.service.js";
...
export function DriverPassengersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [data, assignments] = await Promise.all([
        ticketsService.list(),
        driverAssignmentsService.listMine(),
      ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card
        title={`Direct assignments (${directAssignments.length})`}
        subtitle="Passengers manually assigned to you by an admin"
          ...
          <table className="w-full min-w-[680px] border-collapse text-left text-sm">
            <thead className="bg-slate-100 text-xs uppercase text-slate-600 dark:bg-slate-900/95 dark:text-primary-400/90">
              <tr className="border-b border-slate-200 dark:border-primary-900/40">
                ...
                <th className="px-2 py-2 font-semibold">Assigned by</th>
                <th className="px-2 py-2 font-semibold">Assigned at</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800/90">
              {directAssignments.length === 0 ? (
                <tr>
                  ...
                  >
                    No direct passenger assignments yet
                  </td>
                </tr>
              ) : (
                directAssignments.map((a) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-2 text-slate-600 dark:text-slate-400">
                      {a.assigned_by_name || "Admin"}
                    </td>
                    <td className="whitespace-nowrap px-2 py-2 text-slate-600 dark:text-slate-500">
