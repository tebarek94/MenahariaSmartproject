...
export function UserTicketsPage() {
  ...
  const handleDownloadTicket = async (ticketId) => {
    ...
    try {
      ...
      const downloadToken =
        tokenResponse?.download_token ?? tokenResponse?.data?.download_token;
      ...
      const downloadUrl = ticketsService.getDownloadUrl(downloadToken);
      const res = await fetch(downloadUrl);
      if (!res.ok) {
        let msg = "Failed to download ticket";
        try {
      ...
      setError(err?.data?.message || err?.message || "Failed to download ticket");
    } finally {
              ...
              <Card key={ticket.id} className="p-6">
                <div className="flex justify-between items-start gap-4">
                  <div className="min-w-0 space-y-2">
                    ...
                    <p className="text-slate-400 text-sm">
                      Driver: {ticket.driver_name || "Not assigned"}
                    </p>
                    <p className="text-slate-400 text-sm">
                      Driver phone: {ticket.driver_phone || "Not available"}
                    </p>
