...
function isNetworkFetchError(err) {
  ...
  return /failed to fetch|networkerror|load failed|aborted|timeout/i.test(m);
}
...
async function request(path, { method = "GET", body, headers = {} } = {}) {
  ...
  try {
    ...
    if (isNetworkFetchError(err)) {
      const hint =
        import.meta.env.DEV
          ? "Cannot reach the API. Start the backend (port 5000) and refresh — e.g. `npm start` in the `backend` folder."
